...
import { Brain, CalculatorIcon, ChevronRight, CloudUploadIcon, File, LayoutDashboardIcon, LogOut, NotebookPenIcon, User } from 'lucide-react'
...
const handleLogout = async () => {
  ...
  return (
    <div className={`pt-22 flex flex-col gap-2.5 w-70 h-screen max-md:hidden p-10 ${dark ? 'bg-slate-950 text-white' : 'bg-slate-100 text-black' }`}>
      ...
      <NavLink to={'/upload'} className={`NavLink font-bold rounded flex gap-1.5 p-2.5 ${dark ? 'hover:bg-[#601b9b]' : 'hover:bg-slate-300'}`}> <CloudUploadIcon className=''/> Upload</NavLink>
